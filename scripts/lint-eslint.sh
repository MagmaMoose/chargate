...
case "$rc" in
  0) log_ok "ESLint: clean"; exit "$CINNABAR_OK" ;;
  1) log_error "ESLint reported problems"; gh_error "ESLint reported problems"; exit "$CINNABAR_FINDINGS" ;;
  *) log_warn "ESLint failed to run (exit $rc) — not counted as a finding"; gh_warning "ESLint failed to run (exit $rc)"; exit "$CINNABAR_TOOLERR" ;;
esac
